...
Open [http://localhost:8000/evolution-tree-3d.html](http://localhost:8000/evolution-tree-3d.html) for the **3D Evolutionary Motion Tree**. This view is one self-contained HTML page with inline styles and controls, eliminating separate page-specific CSS and JavaScript files that could become stale. The same 117-node average-linkage hierarchy grows from a shared root to all 59 animated observed movements in a plant-like spatial crown. A visual backbone is traced by repeatedly selecting the child with the largest descendant set. Its first ten parent–child steps are fixed to the vertical axis and kept neutral grey, producing a straight lower trunk that remains upright as the view rotates. Above those ten steps, the backbone continues with very little horizontal travel until it reaches the first fork whose smaller child contains at least 65% as many descendants as the larger child. In the current hierarchy this occurs at depth 15, where the two top subtrees contain 12 and 10 movements.

The crown is deliberately not laid out as one flat left–right bifurcation. The five side subtrees that leave the backbone after depth 10 and the two children of the balanced top fork are treated as seven major crown launches. They are ranked by descendant count and interleaved around seven evenly spaced azimuth sectors, with small deterministic perturbations, so large boughs occupy different directions around the full 360-degree crown. Earlier, smaller trunk shoots use a depth-ordered golden-angle phyllotactic sequence. The largest child at the top fork becomes a nearly vertical continuation of the crown leader, while the other top child opens broadly; subsequent leader edges preserve directional continuity but accumulate deterministic torsion, and lateral children diverge progressively more at higher levels. This produces multiple overlapping primary boughs instead of two isolated top arms.

Branch elevation remains upward-growing but intentionally varied: the crown leader starts with a vertical ratio of approximately 0.992–0.996; ordinary leaders range approximately 0.65–0.83; trunk-launched boughs range approximately 0.58–0.74; and lateral shoots range approximately 0.56–0.75. Segment length varies independently by child around a 1.78-unit base and increases with hierarchy depth. Eligible later branches receive a deterministic additional 12–50% reach extension, producing long scaffold boughs and shorter interior twigs. A 2.65 horizontal expansion factor creates the broad crown, while post-straight backbone segments retain a smaller 0.5 factor. Connections use tangent-aware cubic curves: leader edges enter each joint along the incoming growth direction, whereas lateral edges arc away with a deterministic 7–14% bend and 2.5–6% upward lift. Every parent, child, and endpoint remains unchanged, and the straight lower stem is explicitly exempt from curvature.

Tips with more root-to-tip hierarchy steps generally appear taller; selecting an avatar reports its step count. Height therefore approximates topological hop depth in this rendering, not dendrogram dissimilarity. Branch width still tapers with descendant-cluster size. The rotatable hierarchy is projected into SVG, while each animated leaf uses the same `model-viewer` engine as the embedding and circular views. Every GLB is normalized by its frame +5 Head-to-Hips skeleton measurement, centered using the 64-phase skeleton track, and played at 3× speed. Drag to rotate, scroll to zoom, use the branch navigator to isolate a continuous root-to-canopy sector, or choose a numbered avatar to trace one terminal path. The pinned avatar component and Draco decoder are vendored locally, so the hierarchy, renderer, decoder, and GLBs all load from the same local server. Hop depth in this inferred similarity tree is not evidence of historical chronology, biological evolution, cultural primitivity, or direct descent.
